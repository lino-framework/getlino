...
class DockerTests(TestCase):
    def run_commands_for(self, docker_tag):
        if docker_tag:
            client.containers.run(docker_tag, "pip3 install -e .")
            client.containers.run(docker_tag, "getlino configure --batch --db-engine postgresql && getlino startsite noi mysite1 --batch --dev-repos 'lino noi xl'")
